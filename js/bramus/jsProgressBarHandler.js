		...
		JS_BRAMUS.jsProgressBar.prototype = {
			 ...
			 */
				...
				initialize		: function(el, percentage, options) {
					...
					this.initialPerc	= 0;					// Store this, we'll need it later.
			 ...
			 */
				...
				processQueueEntry	: function(targetPercentage) {
					...
					if ((targetPercentage.toString().substring(0,1) == "+") || (targetPercentage.toString().substring(0,1) == "-")) {
						targetPercentage	= curPercentage + parseFloat(targetPercentage.toString().substring(1,targetPercentage.toString().length));
					}
			 ...
			 */
				_setBgPosition		: function(percentage) {
					// adjust the background position
						$(this.id + "_percentImage").style.backgroundPosition 	= Math.round(this.initialPos + (percentage * this.pxPerPercent))+ "px 50%";
